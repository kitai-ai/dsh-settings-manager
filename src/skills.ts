...
export function skillRoots(env: NodeJS.ProcessEnv = process.env): SkillRoot[] {
...
function readSkillFile(path: string): { description: string; whenToUse?: string; body: string } {
  const raw = readFileSync(path, 'utf8')
  const match = FRONTMATTER.exec(raw)
  if (match === null) return { description: '', body: raw }
...
function skillFileFor(root: string, name: string): { path: string; kind: 'bundle' | 'flat' } | undefined {
  const bundle = join(root, name, 'SKILL.md')
  if (existsSync(bundle)) return { path: bundle, kind: 'bundle' }
  const flat = join(root, `${name}.md`)
  if (existsSync(flat)) return { path: flat, kind: 'flat' }
  return undefined
}

/** List every skill across the standard roots, newest first within each root. */
export function listSkills(env?: NodeJS.ProcessEnv): { roots: SkillRoot[]; skills: SkillEntry[] } {
...
export function getSkillBody(root: string, name: string): SkillResult<SkillBody> {
  if (!SKILL_NAME.test(name)) return { ok: false, error: '技能名必须是 kebab-case（小写字母和数字，用 - 连接）' }
  const target = skillFileFor(root, name)
  if (target === undefined) return { ok: false, error: `未找到技能 "${name}"` }
  try {
...
export function upsertSkill(input: UpsertSkillInput): SkillResult<SkillBody> {
  const name = input.name.trim()
  if (!SKILL_NAME.test(name)) return { ok: false, error: '技能名必须是 kebab-case（小写字母和数字，用 - 连接）' }
  const description = input.description.trim()
  if (description === '') return { ok: false, error: 'description 是必填的，模型目录靠它路由' }
  try {
  ...
  const target = existing ?? { path: join(input.root, name, 'SKILL.md'), kind: 'bundle' as const }
  ...
  if (input.whenToUse !== undefined && input.whenToUse.trim() !== '') meta.whenToUse = input.whenToUse
  let front: string
  try {
...
export function deleteSkill(root: string, name: string): SkillResult<{ removed: boolean }> {
  if (!SKILL_NAME.test(name)) return { ok: false, error: '技能名必须是 kebab-case' }
  const target = skillFileFor(root, name)
  if (target === undefined) return { ok: false, error: `未找到技能 "${name}"` }
  try {
    if (target.kind === 'bundle') rmSync(join(root, name), { recursive: true, force: true })
